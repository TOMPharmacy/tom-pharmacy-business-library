# Receive, hold, move, return, and recall stock
...
**Not:** clinical protocol or guarantee that following steps prevents all loss
...
Connect physical inventory movements to documentary evidence so receiving, holds, returns, recalls, and CS discrepancy reporting stay auditable. Clocks for significant loss and DSCSA suspect product are hard stops.

## Who should use it / who else to involve
Inventory lead (process owner); PIC (CS verification); designated reporter for Form 106; quality lead for CAPA.

## How to use the companion tools
Execute with `LIB08-05` templates; escalate L3 questions via `LIB08-03`; measure with `LIB08-07`.

## Core content — end-to-end workflow
1. **Select supplier** → confirm ATP / license-registration [5]; record in ATP log.  
2. **Purchase order** → match expected NDCs, quantities, cold-chain flags.  
3. **Receive** → invoice vs PO; CS sealed count; **pharmacist initials + actual receipt date** on CS invoices (TX Class C pattern) [7].  
4. **Hold / quarantine** → suspect product (DSCSA) [5]; temperature excursion (SPC editorial); variance pending investigation.  
5. **Stock / move** → secure storage; dual-control for high-risk SKUs (policy); ADC restock verification (ASC).  
6. **Dispense / distribute** → separate CS recordkeeping rules [3].  
7. **Return / recall** → document lot/serial as system supports; trading-partner notifications per DSCSA when applicable [5][6].  
8. **Count** → perpetual (C-II Class C) [7]; cycle; biennial federal inventory [3].  
9. **Investigate variance** → book vs count; camera/access review; determine theft vs significant loss vs documentation error.  
10. **Report** → Field Division ≤1 business day + Form 106 if theft/significant loss [2]; board notices as required.  
11. **CAPA** → access, cameras, purchasing limits, staffing, training [8][9].

### Stop conditions
- Suspect illegitimate product → do not restock for dispensing until investigation path complete [5].  
- Significant CS loss discovered → start DEA clock immediately [2].  
- Missing pharmacist receipt initials on CS invoice (Class C) → treat as documentation defect requiring PIC follow-up [7].
