# Purchasing terms and inventory cash scenarios
...
**Not:** valuation advice, guaranteed margins, or 340B arbitrage schemes
...
Translate supplier terms, payment timing, shrink, and CS variance into **local** cash scenarios you fill with your own numbers. This workbook does not prescribe wholesaler contracts or invent industry benchmarks.

## Who should use it / who else to involve
Owner + PIC + bookkeeper/accountant; inventory lead for count data; counsel before any 340B model changes (**HELD**).

## How to use the companion tools
Pair with `LIB08-05` receiving worksheets and `LIB08-07` variance metrics. Feed cash outputs into Topic 04 payment packs if used.

## Core content
### A. Turns & shrink worksheet (synthetic $ — enter local figures)
| SKU class | Beg $ | Purch $ | End $ | COGS $ | Shrink $ | Notes |
|-----------|-------|---------|-------|--------|----------|-------|
| Non-CS retail | | | | | | COM |
| C-II | | | | | | Perpetual / cycle |
| C-III–V | | | | | | Readily retrievable |
| Specialty cold-chain | | | | | | SPC float |
| ASC floor stock / ADC | | | | | | ASC |

**COGS (synthetic formula):** Beg + Purch − End − Shrink adjustments you define. Do not paste vendor “industry average” tables as facts.

### B. Purchasing terms comparison (blank)
| Supplier | Payment terms | Minimums | Return window | ATP status last checked | Disruption alternative |
|----------|---------------|----------|---------------|-------------------------|------------------------|
| Primary wholesaler | | | | | |
| Secondary | | | | | |
| Specialty distributor | | | | | SPC |

### C. CS variance & cash exposure log
| Date | Drug/NDC | Book qty | Count qty | Δ | Est. $ | Investigator | CAPA # |
|------|----------|----------|-----------|---|--------|-------------|--------|
| | | | | | | | |

### D. Form 106 discovery clock (operational cash + compliance)
| Discovery datetime | Field Division notified (≤1 business day) [2] | Form 106 TLR # | Police # | Insurance claim opened? |
|--------------------|-----------------------------------------------|----------------|----------|-------------------------|
| | | | | |

### E. Disruption scenarios (fill assumptions)
1. Primary wholesaler outage 5 business days → secondary ATP-checked supply cost delta: $____  
2. Cold-chain excursion quarantine (SPC) → lost usable stock $____  
3. CS significant-loss investigation downtime hours × loaded labor $____  
