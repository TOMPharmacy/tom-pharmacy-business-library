# Receiving discrepancy and supplier review records
...
**Not:** filled examples with real PHI, DEA numbers of live pharmacies, or vendor secrets
...
Capture stock exceptions and supply evidence without inventing demand. Blank worksheets for receiving, ATP verification, dual-control access, and theft/loss packet cover sheets.

## Who should use it / who else to involve
Receiving tech/pharmacist; PIC review; inventory lead archives.

## How to use the companion tools
Attach completed sheets to variance CAPAs (`LIB08-07`) and Form 106 packets when applicable [2].

## Core content
### 1. Receiving checklist (COM/ASC)
- [ ] Wholesaler/supplier ATP verified this quarter [5]
- [ ] Invoice matches PO (NDC, qty, lot as available)
- [ ] CS sealed / count verified before stock
- [ ] Pharmacist initials + **actual date of receipt** on CS invoice [7]
- [ ] Temp-sensitive items logged (SPC)
- [ ] Suspect product? → quarantine hold; do not dispense [5]
- [ ] Discrepancy ticket # ________

### 2. ATP verification log
| Date | Partner name | License/reg check source | Result (pass/fail) | Initials | Next due |
|------|--------------|--------------------------|--------------------|----------|----------|
| | | FDA/state board / switch | | | |

### 3. Receiving discrepancy ticket (synthetic fields)
| Field | Entry |
|-------|-------|
| Date/time found | |
| PO # / Invoice # | |
...
| Expected qty | |
| Received qty | |
| Condition (seal/temp/damage) | |
| Quarantine location | |
| PIC notified | Y/N |
| Supplier RMA / credit # | |
| Closed date | |

### 4. Theft/loss packet cover sheet
Pharmacy DEA #: ________ (synthetic in drills)  
Discovery datetime: ________  
Field Division notice sent: ________ (**must be ≤1 business day** of discovery) [2]  
Form 106 TLR confirmation #: ________  
Police report #: ________  
Board notice (if required): ________  
Internal CAPA #: ________  

### 5. Dual-control CS access log
| Date/time | Staff A | Staff B | Purpose | NDC/qty | Initials |
|-----------|---------|---------|---------|---------|----------|
| | | | | | |

### 6. Supplier quarterly review (blank)
| Criterion | Primary | Secondary | Notes |
|-----------|---------|-----------|-------|
| On-time fill % (local data) | | | |
| Credit/return friction | | | |
| ATP documentation quality | | | |
| Cold-chain incident count | | | SPC |
| Decision: keep / dual-source / exit | | | |
