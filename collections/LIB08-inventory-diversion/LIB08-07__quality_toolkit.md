# Stock reliability, expiry, and recall response review
...
**Not:** arbitrary national KPI targets or guaranteed recall performance
...
Track avoidable exposure (variance, late investigations, missing receipt initials, open recall actions) and close CAPAs with effectiveness checks. Targets are **local**—do not invent industry benchmarks.

## Who should use it / who else to involve
Quality lead + PIC; inventory lead; owner for resource decisions.

## How to use the companion tools
Monthly review sheet; escalate DEA/DSCSA clocks via workflow `LIB08-04`.

## Core content
### Metric dictionary (define locally)
| Metric | Definition (fill) | Data source | Owner |
|--------|-------------------|-------------|-------|
| CS variance rate | Variances / 1000 doses or / cycle | Perpetual / cycle logs | PIC |
| Days to close variance | Discovery → CAPA close | CAPA log | Quality |
| % CS invoices with Rx initials | Initialed / total CS invoices | Receiving files [7] | Inventory |
| Open recall actions >X days | Count | Recall tracker | Inventory |
| Mock Form 106 drill completed | Y/N this year | Drill AAR [2] | PIC |
| ATP checks current | Partners verified / due | ATP log [5] | Inventory |
...
### Annual drill (synthetic)
Simulate missing C-II bottle → complete discovery clock worksheet → draft Field Division notice (**do not transmit in drill**) → after-action: time to assemble packet, camera retrieval success, dual-control gaps [2][8][9].

### Review cadence
- Weekly: open CS variances >48h  
- Monthly: metric dictionary + CAPA aging  
- Quarterly: supplier ATP + terms review  
- Annually: Form 106 tabletop + DSCSA suspect-product tabletop  
