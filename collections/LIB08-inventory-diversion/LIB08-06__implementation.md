# Barcode tracing and inventory system rollout
...
**Not:** product endorsement or validated clinical barcode protocol
...
Prepare roles, test evidence retrieval, and rehearse stock movements before going live on barcode/DSCSA/perpetual modules. Acceptance tests use **synthetic** NDCs and quantities only.

## Who should use it / who else to involve
PIC (clinical ops acceptance); inventory lead; IT/vendor; privacy official if cameras or user logs touch PHI (Topic 07 / LIB07).

## How to use the companion tools
Run acceptance checklist; store evidence with `LIB08-05` logs; score reliability in `LIB08-07`.

## Core content
### Role & control matrix
| Control | Owner | Tool / evidence | Setting |
|---------|-------|-----------------|---------|
| Badge/key CS area | PIC | Access list + alarm test log | COM ASC |
| Camera retention policy | Owner | NVR retention days documented | COM |
| Perpetual C-II module | PIC | System report export | ASC Class C [7] |
| DSCSA EPCIS/trace inbox | Inventory lead | Switch/vendor ticket | COM SPC [5] |
| Delivery seal numbers | Ops | Scanner + custody sheet | TEL |
| ADC restock witness | Nursing + Rx | ADC report + signature | ASC |

### Synthetic acceptance test script
1. Create test PO with fictional NDC; receive; confirm pharmacist initial field captured [7].  
2. Force a qty mismatch; confirm discrepancy ticket and quarantine bin workflow.  
3. Export CS perpetual report; confirm Schedule II separable from III–V [3].  
4. Retrieve ATP check record for primary wholesaler [5].  
5. Tabletop: missing C-II → discovery clock draft notice (**do not send**) [2].  
6. Confirm camera/access logs available for the test window without exporting real PHI to shared folders.

### Rollout gates
- [ ] BAA / vendor security questionnaire complete if ePHI in scope (LIB07)  
- [ ] Dual-control policy signed  
- [ ] Staff competency checklist for receiving  
- [ ] Mock Form 106 drill scheduled within 90 days of go-live  
