# Deploy pharmacy technology across hospital workflows
...
Plan interface, role, and downtime tests with synthetic data. Orient to common tech patterns from ASHP survey (ADC primary distribution, machine-readable checks, autoverification prevalence) without treating survey % as requirements [7].

## Who should use it / who else to involve
Informatics; nursing education; IT security; PIC.
...
## Core content — test matrix
| Test | Synthetic case | Roles | Pass |
|------|----------------|-------|------|
| ADC stock-out | High-use med zeroed | Tech + RPh + RN | Bypass documented |
| Autoverify off | Toggle downtime | RPh | Manual verify queue clears |
| Label scan fail | Bad barcode | Tech | Quarantine path |
| Diversion discrepancy | Count mismatch | PIC + compliance | Investigation started [5] |
