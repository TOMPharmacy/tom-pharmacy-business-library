# Choose technology from a pharmacy problem
...
**Risk tier:** L1 (selection) / L3 (HIPAA/DEA clusters)  
**Version:** 0.3-final-draft  
**Last verified:** 2026-09-19  
**Not:** legal advice, product endorsement, clinical validation of software, or compliance certification
...
Start from a **pharmacy problem** (downtime risk, e-Rx hazards, access creep, missing audit logs)—not from a vendor demo. Decide whether you are a HIPAA covered entity [1], what Security Rule risk analysis and contingency planning require [3], how Breach Notification clocks run [4], and how Texas Class C backup/downtime rules intersect [6]. Pair OCR primary pages with peer-reviewed e-prescribing safety literature when selecting or configuring PMS/e-Rx stacks [7][8][9].

## Who should use it / who else to involve
Designated **security official** [3]; PIC; counsel for BAAs [1]; vendor managers for MFA and audit-log export commitments; privacy trainer for workforce.
...
## Core content
### Covered entity / business associate gate
Pharmacies are listed among providers that are HIPAA covered entities if they transmit health information electronically in connection with a standard transaction [1]. Written BA contracts are required when engaging business associates; BAs have direct liability for certain provisions [1].

### Security Rule anchors (shall-level orientation)
Ensure confidentiality, integrity, and availability of ePHI; protect against reasonably anticipated threats and impermissible uses/disclosures; ensure workforce compliance [3]. Perform an accurate and thorough **risk analysis** [3]. Contingency plan: backup, restore, emergency-mode operations [3]. Documentation retained **six years** [3]. Designate a security official [3].

### Breach Notification
Following a breach of unsecured PHI, notify individuals, the Secretary, and in certain circumstances the media [4]. Business associates notify the CE without unreasonable delay and no later than **60 days** from discovery [4].

### HIT safety evidence (practice, not product picks)
E-prescribing can introduce new medication-error risks if poorly designed [7]. Sociotechnical hazards span technology, people, organization, and external environment [8]. Pharmacist surveys document observed HIT-related errors/near misses—use to scope SRA workshops [9].

### Synthetic vignette
“Oak Street Pharmacy” delays PMS cutover because the vendor cannot export audit logs or sign a BAA. Selection scorecard fails gate—price is irrelevant until those evidence rows pass.
