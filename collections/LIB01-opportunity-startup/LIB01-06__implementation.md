# People systems and premises launch rehearsal
...
Coordinate initial coverage, technology setup, and practical opening exercises so “day one” is a rehearsal with acceptance evidence — not a hope. Texas pre-inspection expects PIC and owner (or representative) involvement along the Rule 291.1(e) path unless waiver criteria apply [6]. DEA registration must be active before controlled-substance handling [7][8].

## Who should use it / who else to involve

PIC leads clinical/ops readiness; owner leads vendors and premises; technician lead runs role drills; IT/vendor admin configures PMS without sharing passwords in this document.
...
| Area | Acceptance evidence | Owner | Done? |
|------|---------------------|-------|-------|
| License / inspection status | Board correspondence | Ops | ☐ |
| DEA (if CS) | Registration certificate on site [7][8] | PIC | ☐ |
| NPI enumerated | NPPES screenshot [13] | Billing | ☐ |
| Wholesaler account | Account # / credit letter on file [5] | Owner | ☐ |
| Premises security | Alarm test log | Owner | ☐ |
| PMS / eRx | Test claim in test environment (synthetic) | IT/PIC | ☐ |
| Staff roster | Named RPh coverage all open hours | PIC | ☐ |
| Counseling space / process | Role-play new Rx counseling (LIB03) | PIC | ☐ |
| Emergency contacts | Escalation card posted | Ops | ☐ |
| Opening inventory count | Signed count sheets | PIC | ☐ |

### Fictional end-to-end rehearsal (synthetic)

**Setting: COM.** Team runs: (1) receive synthetic eRx, (2) product verification, (3) counseling documentation drill, (4) delivery bag temp check if offering delivery, (5) end-of-day CS count if DEA active. Failures open CAPA items — no patient data used.

### Sterile / specialty branch

If A-S / 503A sterile intended [1][10], add competency and environment checks from your primary sterile sources (USP text **held** — do not paste). Class A alone must not silently add sterile [1].
