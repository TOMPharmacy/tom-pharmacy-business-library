# The founder decision and launch handoff process
...
Keep dependencies visible before committing to a launch date. This workflow sequences decide → form entity → license → inspect → enroll identifiers → open, with an accountable owner, evidence gate, and escalation at each step. Texas mid-application changes cancel the filing [4] — treat deal and application calendars as coupled.

## Who should use it / who else to involve

Project lead (often owner) owns the Gantt; PIC owns inspection readiness; counsel owns entity/FEIN freeze; CPA owns cash gates from LIB01-02; billing lead owns NPI/enrollment after license path is real.
...
| Stage | Milestone | Accountable | Evidence gate | Escalate if |
|-------|-----------|-------------|---------------|-------------|
| 0 | Stop/proceed decision | Owner | Signed log; need + class + cash documented | Cash blank; class unclear |
| 1 | Entity / FEIN / SOS | Counsel/CPA | Formation docs | FEIN will change after app start [4] |
| 2 | Class selection freeze | PIC/counsel | Service vs class table [1][2] | Sterile added late |
| 3 | Facility application packet | Ops | Checklist complete [5] | Missing wholesaler letter |
| 4 | Pre-inspection scheduling | PIC/owner | PIC + owner calendar [6] | PIC unavailable |
| 5 | DEA Form 224 (if CS) | PIC | Submission confirmation [8] | State license not ready |
| 6 | NPI (and later payer IDs) | Billing | NPPES status [13] | Assuming NCPDP=NPI [held] |
| 7 | Systems + staff rehearsal | PIC/ops | LIB01-06 checklist | Critical SOP missing |
| 8 | Soft open / first claims | Owner/PIC | License in hand; CS only if DEA active [7][8] | Dispense before authority |

### Handoff rules

- No stage skips without written risk acceptance by owner + PIC.  
- If ownership/officer/location must change after review begins in Texas, plan to **reapply** [4].  
- ASC branch: insert HHSC substantial completion before Stage 3 Class C expectations [12].  
- Tele/Class E: add home-state and ship-to-state tracks in parallel.

### Synthetic example

**Setting: TEL.** Fictional nonresident planner runs Stage 0–2 in home state, opens Texas Class E row [1][2], and refuses a marketing “launch date” until Stage 5–6 identifiers are listed with owners — even though website copy is ready.
