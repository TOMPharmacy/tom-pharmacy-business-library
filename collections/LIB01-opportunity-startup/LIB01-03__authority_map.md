# Opening authority and evidence map
...
Identify entity, site, service, and state permissions that require qualified confirmation before day-one dispense. This is a **reference-only** matrix: cite controlling sources, record retrieval dates, and park unresolved questions. It does not approve a filing and does not replace counsel.

## Who should use it / who else to involve

Owner/PIC assemble the matrix; **state pharmacy counsel** and, where controlled substances or sterile compounding apply, additional qualified reviewers must sign before use. Accountant confirms FEIN/entity facts. ASC/clinic admins join when facility licensure gates apply.
...
### Authority matrix (complete per site)

| Domain | Question | Controlling / primary cite | Evidence to file | Owner | Status |
|--------|----------|---------------------------|------------------|-------|--------|
| Entity | Legal name, FEIN, SOS, franchise tax | Class A checklist categories [5] | SOS print / FEIN letter | Counsel/CPA | |
| Facility class | Which TX class matches services? | §560.051 [2]; TSBP class page [1] | Service description vs class notes | PIC/counsel | |
| Location | Separate license per place of business? | §560.102 [3] | Address / lease | Counsel | |
| Application stability | Will ownership/officer/location change mid-review? | TSBP cancel rule [4] | Deal calendar | Owner | |
| Packet completeness | Forms, disclosure, officers, lease, LIC-018, wholesaler credit, LIC-000A | Class A checklist [5] | Packet index | Ops | |
| Inspection | On-site with PIC + owner unless waiver | Rule 291.1(e)/(f) via checklist [6] | Calendar holds | PIC/owner | |
| Controlled substances | DEA registration per location; Form 224 | Manual + registration page [7][8]; 21 CFR 1301.12 | Form 224 receipt | PIC | |
| FDA pharmacy exemption | Still within 207.13 retail course? | 21 CFR 207.13 [11] | Activity description | Counsel | |
| Compounding regime | 503A vs 503B? | FDA compounding page [10] | Regime election memo | PIC/counsel | |
| Sterile subclass | A-S / C-S needed? | TSBP Class A sterile prohibition [1] | Floor plan / SOP intent | PIC | |
| Clinic Class D | Not a physician office? | TSBP Class D note [14] | Site type memo | Counsel | |
| Central fill Class G | No bulk store / no dispense? | TSBP Class G note [15] | Process map | Ops | |
| Institutional Class C | HHSC substantially complete? | Class C checklist [12] | HHSC status letter | ASC admin | |
| NPI | NPPES/EFI/paper path | CMS [13] | NPI enumeration | Billing | |
| NCPDP | Separate commercial ID? | **Held** 01-C018 | Portal ticket | Billing | |
| Payer enrollment | Network timelines | Deferred LIB05 | — | Revenue | |

### Evidence index method

Number each PDF/screenshot in a local evidence folder (private). Record: evidence_id, authority row, date, URL, who retrieved. Never paste licensed third-party manuals into the pack.

### Synthetic example

**Setting: SPC.** Fictional specialty startup marks 503A + Texas A-S as hypothesis [10][1], opens matrix rows for DEA and sterile subclass, and parks USP operational claims as held. Counsel initials the matrix before any vendor deposit beyond exploratory quotes.
