# Cost routine urgent and transition support
...
**Not:** guaranteed census ROI or survey savings

## What this helps you decide or do
Model deliveries, after-hours, and transition support using local rates. Pair with LIB01/LIB04 methods for overhead. Census and acuity assumptions stay labeled synthetic.

## Who should use it / who else to involve
Pharmacy owner; account manager; accountant; facility administrator (inputs only).
...
## Core content
### A. Service costing addendum
| Activity | Volume/mo | Minutes each | Labor $ | Courier $ | Total |
|----------|-----------|--------------|---------|-----------|-------|
| Cycle fill | | | | | |
| New admit / TOC | | | | | |
| STAT / after-hours | | | | | |
| Monthly DRR (consultant hrs) | | | | — | |
| CS reconciliation support | | | | — | |

### B. Continuity capacity scenarios
| Scenario | Census | STAT rate | Courier OT | Cash impact | Decision |
|----------|--------|-----------|------------|-------------|----------|
| Base | | | | | |
| Flu season +20% STAT | | | | | |
| Lost weekend courier | | | | | |

### C. Exclusions (write explicitly)
List services not included (sterile compounding, REMS specialty, medical equipment) so proposals stay honest.
