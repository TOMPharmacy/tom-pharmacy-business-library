# Facility transitions and ongoing pharmacist review handoffs
...
Route transition work and ongoing consultant review findings without directing therapy or inventing review timing beyond §483.45’s monthly minimum [1]. Urgent irregularities need facility P&P time frames [1].

## Who should use it / who else to involve
Consultant pharmacist; DON; attending; dispensing pharmacy clinical liaison.
...
## Core content — reference-only workflow
1. **Admit/TOC:** collect discharge list (label provisional) → compare to facility orders/MAR → escalate mismatches to facility nursing/prescriber (C12-007 held process tip).  
2. **Monthly DRR:** chart + regimen review → document → if irregularity, **separate written report** to attending, medical director, DON [1].  
3. **Acknowledgment:** track physician documentation of review/action/rationale [1].  
4. **Urgent path:** follow facility P&P steps when irregularity requires urgent protection [1].  
5. **Handback:** dispensing pharmacy acts only on clarified orders — not on consultant narrative alone.
