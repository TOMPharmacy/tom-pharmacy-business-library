# Escalate incidents and restore essential services
...
**Not:** clinical emergency medical protocol
...
Define ownership, evidence preservation, and recovery verification for operational incidents (not medical emergencies).

## Who should use it / who else to involve
Incident commander (PIC or designee); quality lead; facility EP coordinator when under CMS EP [1].
...
## Core content — incident workflow
1. **Detect** — variance, outage, suspected diversion, suspect product, near miss.  
2. **Contain** — quarantine stock; isolate system; secure CS.  
3. **Preserve evidence** — counts, camera window, access logs (no PHI in shared AAR).  
4. **Notify** —  
   - DEA Field Division ≤1 business day if theft/significant loss [6]  
   - Board as required (e.g., Class C ADC downtime rules) [7]  
   - Facility EP/comms tree if CMS EP setting [2]  
   - DSCSA trading partners/FDA if illegitimate [8]  
5. **Restore** — essential services checklist; verify before declaring recovered.  
6. **Learn** — CAPA; classify with NCC MERP index when medication-error related [9].  
7. **Exercise** — schedule next drill; annual EP plan review if applicable [2].

### Stop / escalate
- Life-safety emergency → facility emergency procedures (outside this pack).  
- Unsecured PHI breach → LIB07 Breach Rule path.  
